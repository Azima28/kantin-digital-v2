...
class PosOrderChatSheet extends ConsumerStatefulWidget {
  final OrderItem order;

  const PosOrderChatSheet({super.key, required this.order});
  ...
  static Future<void> show(BuildContext context, {required OrderItem order}) {
    return showModalBottomSheet<void>(
      ...
      builder: (context) => PosOrderChatSheet(order: order),
    );
  }

  @override
  ConsumerState<PosOrderChatSheet> createState() => _PosOrderChatSheetState();
}

class _PosOrderChatSheetState extends ConsumerState<PosOrderChatSheet> {
  ...
  Widget _buildWhatsAppTick(OrderMessage msg, bool isDark, bool isRecipientActive) {
    ...
    if (isLocal) {
      ...
      // Centang 2 Biru (Double Blue Ticks): Sudah dibaca oleh siswa!
      return const Icon(
      ...
      // Centang 2 Abu-abu (Double Grey Ticks): Terkirim & diterima siswa yang sedang aktif
      return Icon(
        Icons.done_all_rounded,
        size: 14,
        color: isDark ? Colors.white60 : Colors.grey,
      ...
      // Centang 1 Abu-abu (Single Grey Tick): Terkirim ke server, siswa sedang offline / tidak aktif
      return Icon(
        Icons.check,
        size: 14,
        color: isDark ? Colors.white60 : Colors.grey,
      ...
      child: Column(
        children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        widget.order.studentName.isNotEmpty
                            ? 'Siswa: ${widget.order.studentName}'
                            : 'Siswa Kantin',
                        style: HallmarkTypography.titleSmall(colors.textPrimary),
                      ),
                      Text(
                        'ID: #${widget.order.id.length > 8 ? widget.order.id.substring(0, 8) : widget.order.id} • ${widget.order.status} • ${isSiswaOnline ? "Online" : "Offline"}',
          ...
          Expanded(
            ...
            // Merchant Quick Reply Chips
            Container(
              height: 44,
              padding: const EdgeInsets.symmetric(horizontal: 12),
              child: ListView.separated(
                scrollDirection: Axis.horizontal,
                itemCount: OrderChatPresets.pedagang.length,
                separatorBuilder: (_, __) => const SizedBox(width: 8),
                itemBuilder: (context, index) {
                  final preset = OrderChatPresets.pedagang[index];
                  return ActionChip(
                    label: Text(
                      preset,
