...
def main():
    ...
    # Schema (DDL only) is tracked in git and is what docker-compose mounts into
    # /docker-entrypoint-initdb.d. The row data goes to a separate, gitignored
    # file because it carries real accounts, bcrypt password hashes and
    # session tokens.
    schema_out = "database_backup/init_postgres_standalone.sql"
    data_out = "database_backup/json_seed_data.sql"
...
"""
    ...
    lines = [
        "-- =====================================================================",
        "-- KANTIN DIGITAL v2.0 - SEED DATA GENERATED FROM database_backup/json",
        "-- CONTAINS REAL ACCOUNTS, BCRYPT PASSWORD HASHES AND SESSION TOKENS.",
        "-- This file is gitignored on purpose. Do not commit it.",
        "-- Restore with:",
        "--   psql -U postgres -d kantin_digital -f database_backup/json_seed_data.sql",
        "-- =====================================================================",
    ]
    ...
    print(f"[SUCCESS] Schema (safe to commit) generated: {schema_out}")
    print(f"[SUCCESS] Seed data (gitignored, contains PII) generated: {data_out}")
