-- Migration: Add p_caller_id fallback to process_topup
-- Tujuan: Izinkan top-up tanpa session token jika caller_id cocok dengan student_id (self-topup)

DROP FUNCTION IF EXISTS public.process_topup(UUID, BIGINT, TEXT, TEXT, TEXT) CASCADE;
...
BEGIN
    ...
    -- 2. Fallback: gunakan p_caller_id jika token tidak valid
    ...
    -- 3. Role check: super_admin, admin, petugas_keuangan, parent yang terhubung ke siswa, atau siswa itu sendiri
    ...
    -- 6. Lakukan penambahan saldo
    v_new_balance := v_old_balance + p_amount;
    UPDATE public.students SET balance = v_new_balance WHERE id = p_student_id;

    -- 7. Cari ID operator kantin valid untuk kepatuhan Foreign Key pada tabel transactions
    ...
    -- 8. Catat transaksi pembayaran & audit log
    INSERT INTO public.transactions (student_id, operator_id, total_amount, type, status)
    VALUES (p_student_id, v_real_operator_id, p_amount, 'topup', 'success');
...
GRANT EXECUTE ON FUNCTION public.process_topup(UUID, BIGINT, TEXT, TEXT, TEXT, UUID) TO authenticated, anon, public;
